...
AgenticNotch で作業するときのガイドです。`~/.agents/AGENTS.md`（グローバル）の指示に加えて、このリポジトリでは以下に従ってください。
...
セルフマージなので PR は必須ではありません。まとまった変更や後から経緯を追いたい変更のみ PR を作ります。PR を作った場合、**マージはユーザーが行います。エージェントは自律的にマージしません。**

## コミット規約

- **コミットは作業単位ごとに切る。** 独立して動作確認できる修正が 1 つ完成したら、その場でコミットしてよい（明示指示は不要）
- **push・PR の作成・マージは、ユーザーの明示指示がない限り行わない**
...
- 末尾に、使用したエージェントの `Co-Authored-By:` トレーラを付ける
